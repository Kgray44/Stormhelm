...
def default_adapter_contract_registry() -> AdapterContractRegistry:
    ...
    registry.register_contract(
        AdapterContract(
            ...
            family="relay",
            description=(
                "Uses the local Discord client with preview-first dispatch and runtime capability gates "
                "for focus, DM navigation, payload insertion, send submission, and post-send verification."
            ),
            observation_modes=["current_context", "thread_verification", "runtime_capability_report"],
            action_modes=[
                "can_preview",
                "can_dispatch",
                "can_focus_client",
                ...
                "can_report_failure",
                "send_via_local_client",
            ],
            artifact_modes=["preview_payload", "dispatch_trace"],
            preview_modes=["mandatory_preview"],
            safety_posture=[
                "preview_required",
                "truthful_delivery_limits",
                "no_discord_user_token_api",
                "no_self_bot_route",
                "runtime_capability_gate",
            ],
            failure_posture=[
                "stale_preview_refusal",
                "wrong_thread_refusal",
                "duplicate_suppression",
                "dispatch_unavailable_explicit",
                "verification_unavailable_explicit",
                "transport_failure_explicit",
            ],
            trust_tier=TrustTier.EXTERNAL_DISPATCH,
            approval=ApprovalDescriptor(
                required=True,
            ...
            verification=VerificationDescriptor(
                posture="runtime_capability_bounded",
                max_claimable_outcome=ClaimOutcome.VERIFIED,
                evidence=[
                    "local client route",
                    "final send gesture evidence",
                    "post-send observed message evidence",
                    "verification strength",
                ],
                notes=[
                    "sent_verified requires runtime can_verify_send evidence.",
                    "sent_unverified requires the final local send gesture.",
                    "stub or unavailable adapters cannot produce sent states.",
                ],
            ),
            rollback=RollbackDescriptor(supported=False, posture="none"),
