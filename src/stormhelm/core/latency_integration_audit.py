...
def build_latency_integration_audit() -> LatencyIntegrationAudit:
    ...
    items: list[LatencyIntegrationInventoryItem] = [
        ...
        _item(
            ...
            current_status=AuditStatus.LIVE_USED,
            evidence=("OpenAI streaming code path and buffered-projection labels are unit-tested with injected transports.", "Opt-in L5.4 speaker smoke measured true_http_stream first chunk before stream completion."),
            missing=("Live OpenAI smoke remains explicitly gated so CI and normal tests do not spend provider credits.",),
            risk="Low; provider streaming is normal-path behavior when configured, while live network proof remains opt-in.",
            action=AuditRecommendation.KEEP,
        ),
        _item(
            ...
            models_or_functions=("VoiceLivePlaybackRequest", "VoiceLivePlaybackSession", "LocalPlaybackProvider.start_stream", "WindowsMCIPlaybackBackend.start_stream"),
            config_flags=("voice.playback.streaming_enabled", "voice.playback.streaming_fallback_to_file"),
            runtime_entrypoints=("VoiceService.stream_core_approved_spoken_text", "LocalPlaybackProvider.start_stream", "WindowsMCIPlaybackBackend.start_stream"),
            normal_path_usage="Windows local provider accepts progressive PCM chunks from the Core-approved streaming path and plays them through the default output device when voice/playback gates are enabled.",
            test_coverage=("tests/test_latency_l5_voice_streaming_first_audio.py", "tests/test_voice_playback_provider.py", "tests/test_latency_l55_windows_voice_runtime.py"),
            status_surface=("voice.status_snapshot.playback", "voice.playback.speaker_backend_available"),
            trace_fields=("voice_playback_start_ms", "voice_partial_playback"),
            kraken_fields=("voice_partial_playback_count",),
            current_status=AuditStatus.LIVE_USED,
            evidence=("WinMM waveOut progressive PCM speaker streaming is wired through LocalPlaybackProvider.", "L5.5 tests prove speaker truth reaches status/UI without raw audio exposure."),
            missing=("Non-Windows speaker streaming remains unsupported/out of scope.",),
            risk="Low on Windows; unsupported platforms report typed unavailable/unsupported state.",
            action=AuditRecommendation.KEEP,
        ),
        _item(
