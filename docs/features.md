...
- Tests: `tests/test_screen_awareness_service.py`, `tests/test_screen_awareness_phase12.py`, `tests/test_screen_awareness_verification.py`, `tests/test_screen_awareness_action.py`, `tests/test_screen_awareness_playwright_adapter_scaffold.py`, `tests/test_screen_awareness_playwright_adapter_integration.py`, `tests/test_screen_awareness_playwright_canonical_kraken.py`, `tests/test_screen_awareness_playwright_live_semantic.py`, `tests/test_screen_awareness_playwright_grounding_guidance.py`, `tests/test_screen_awareness_playwright_grounding_robustness.py`, `tests/test_screen_awareness_playwright_semantic_verification.py`, `tests/test_screen_awareness_playwright_action_preview.py`, `tests/test_screen_awareness_playwright_click_focus_execution.py`, `tests/test_screen_awareness_playwright_task_plans.py`, `tests/test_live_browser_integration.py`, `tests/test_live_browser_provider_smoke.py`
- Status: Implemented but limited

### Camera Awareness

- What it does: Routes explicit camera/physical-object requests to the Camera Awareness subsystem, performs opt-in single-frame local camera capture when enabled, reports typed unavailable/blocked states when hardware, permission, privacy policy, or config prevents capture, and keeps image evidence bounded to frame metadata/safe artifact refs by default.
- Use it with: Requests like "what can you see through the camera?", "is the camera working?", "take a camera snapshot", "can you identify this connector?", or "does this solder joint look okay?"
- Inputs: Operator text, explicit live camera gates, selected device index, bounded still-capture request, optional explicit artifact-save gate.
- Outputs: Camera capability report, capture result, cleanup status, observation/policy result, event metadata, and Camera Kraken artifacts. Raw frames are not emitted in events/status/reports by default.
- Settings: `camera_awareness.*` plus live-test gates `STORMHELM_LIVE_CAMERA_TESTS`, `STORMHELM_ENABLE_LIVE_CAMERA`, `STORMHELM_CAMERA_REQUIRE_REAL_DEVICE`, `STORMHELM_CAMERA_DEVICE_INDEX`, `STORMHELM_CAMERA_CAPTURE_TIMEOUT_MS`, and `STORMHELM_CAMERA_SAVE_ARTIFACTS`.
- Edge cases: A detected device is not treated as working until a real frame is captured. Missing hardware, OS permission denial, device busy, timeout, empty frame, disabled config, or privacy block returns typed unavailable/blocked state. Identity, emotion, health/age/sensitive inference, continuous watching, video recording, room-safety verification, and camera-based action verification remain unsupported.
...
- Tests: `tests/test_camera_awareness_live_kraken.py`, `tests/test_camera_awareness_planner.py`, `tests/test_camera_awareness_c8_hardening_audit.py`
- Status: Implemented but limited
