...
| `powershell -ExecutionPolicy Bypass -File .\scripts\setup_live_browser_dependencies.ps1` | Check optional Obscura/Playwright live-diagnostic dependencies; install Playwright package/Chromium or Obscura from explicit official release discovery, zip, or URL only when explicit install flags are supplied. | `scripts/setup_live_browser_dependencies.ps1` | `tests/test_live_browser_integration.py`, `tests/test_live_browser_dependencies.py` |
| `powershell -ExecutionPolicy Bypass -File .\scripts\run_live_browser_checks.ps1` | Run opt-in live Obscura CLI/CDP and Playwright semantic diagnostics behind environment gates. The Playwright smoke uses the Screen Awareness adapter's isolated semantic snapshot path when launch is explicitly allowed and reports bounded partial-observation limitations. It does not enable click/focus/type/choice/scroll action gates. Use the bypass form if local policy blocks direct script execution. | `scripts/run_live_browser_checks.ps1`, `src/stormhelm/core/live_browser_integration.py`, `src/stormhelm/core/screen_awareness/browser_playwright.py` | `tests/test_live_browser_integration.py`, `tests/test_live_browser_provider_smoke.py`, `tests/test_screen_awareness_playwright_live_semantic.py`, `tests/test_screen_awareness_playwright_grounding_robustness.py` |
| `python -m stormhelm.core.live_browser_integration` | Backend runner for bounded live browser integration reports. | `src/stormhelm/core/live_browser_integration.py` | `tests/test_live_browser_integration.py` |
| `python scripts\run_camera_awareness_kraken.py --output-dir .artifacts\kraken\camera-awareness-live-01 --require-real-camera --enable-live-camera` | Run the opt-in live Camera Awareness Kraken. Requires real local camera capture when `--require-real-camera` is set; missing hardware/permission returns typed unavailable/blocked, not fake success. | `scripts/run_camera_awareness_kraken.py`, `src/stormhelm/core/kraken/camera_awareness_live.py` | `tests/test_camera_awareness_live_kraken.py` |
...
| `use Obscura CDP to inspect https://example.com/docs` | web_retrieval | Obscura CDP provider | Start a bounded localhost CDP diagnostic/session path only when enabled. If navigation is unsupported, return `fallback_available` / `cdp_navigation_unsupported` and recommend Obscura CLI instead of faking inspection. | No | CDP session trace and `headless_cdp_page_evidence` |
| `show me network summary for this public page https://example.com/docs` | web_retrieval | Obscura CDP provider | Route only when extraction/inspection is requested and CDP page inspection is supported; otherwise report diagnostic-only/fallback state. It does not open or control the visible browser. | No | Bounded network/console summary or unsupported trace |
| `scripts/smoke_obscura_cdp.ps1 -Url https://example.com` | developer/manual | Obscura CDP compatibility probe | Opt-in local smoke check for binary/version/startup/endpoints/websocket cleanup and optional safe URL inspection. Not part of normal CI. | No | Bounded JSON compatibility report |
| `powershell -ExecutionPolicy Bypass -File scripts/run_live_browser_checks.ps1 -Enable -Obscura -ObscuraCdp -Playwright -AllowPlaywrightBrowserLaunch -Url https://example.com` | developer/manual | Live browser integration diagnostics | Opt-in local smoke checks for Obscura CLI, Obscura CDP, and Playwright semantic readiness/snapshot extraction. Playwright launch requires its separate launch flag, uses an isolated temporary context, and returns semantic observations only. | No | Bounded JSON report with claim ceilings and disabled action posture |
...
| `what can you see through the camera?` | camera_awareness | Camera awareness | Use an explicitly gated real camera still when live capture is enabled; otherwise return typed unavailable/blocked. It does not claim continuous watching or identity/emotion inference. | Config/live gate | Real frame metadata and cleanup state |
| `is the camera working?` | camera_awareness | Camera preflight/capture | Treat the camera as working only after a frame is captured. Device listing alone is not enough. | Config/live gate | Capability report and capture latency |
| `keep watching the camera` | camera_awareness | Camera safety policy | Refuse continuous surveillance/background monitoring. | No | Unsupported/refused state |
| `can you identify who is in frame?` | camera_awareness | Camera safety policy | Refuse identity/face-recognition claims. | No | Unsupported/refused state |
| `am I smiling?` | camera_awareness | Camera safety policy | Refuse emotion/sensitive inference. | No | Unsupported/refused state |
| `save this camera image` | camera_awareness | Camera artifact policy | Block unless explicit artifact-save policy/gate is enabled; normal reports use safe refs, not raw frames. | Yes/config gate | Artifact ref only when explicitly saved |
| `find the email field in this isolated browser observation` | screen_awareness | Playwright browser semantic adapter | Ground a target by role/name/label/text/state from the latest semantic snapshot and return guidance only, including synonym/negation hints and ambiguity when needed. | No | Candidate confidence, evidence/mismatch terms, ambiguity state |
| `what fields are on this browser page` | screen_awareness | Playwright browser semantic adapter | Summarize bounded visible form-like controls, required/readonly/disabled fields, unchecked required checkboxes, possible submit controls, links, warnings, and partial-observation limitations from semantic observation. | No | Redacted form/dialog summary and `browser_semantic_observation` claim ceiling |
| `did the warning disappear between these browser snapshots` | screen_awareness | Playwright browser semantic comparison | Compare two existing semantic observations and classify whether the expected outcome is supported, unsupported, partial, ambiguous, stale, or unverifiable. No browser action is performed. | No | Bounded change list and `browser_semantic_observation_comparison` claim ceiling |
| `preview clicking the Continue button in this browser observation` | screen_awareness | Playwright browser action preview | Build an action preview/plan from a grounded semantic target, including action kind, risk, approval posture, required capability, and expected before/after semantic check. Preview alone never executes. | No | Redacted plan and `browser_semantic_action_preview` claim ceiling |
| `click the Continue button in this isolated Playwright browser fixture` | screen_awareness | Playwright browser action execution | Execute click only when Playwright action gates, fresh target grounding, exact trust approval, isolated browser launch, and unambiguous locator checks all pass. It captures before/after semantic observations and returns a precise attempted/verified/unsupported status. | Yes | `browser_semantic_action_execution` plus semantic comparison; not visible-screen or source-truth proof |
| `focus the Email field in this isolated Playwright browser fixture` | screen_awareness | Playwright browser action execution | Execute focus only when Playwright action gates, fresh target grounding, exact trust approval, isolated browser launch, and unambiguous locator checks all pass. It does not type or read field values. | Yes | `browser_semantic_action_execution`; expected focus change may be `completed_unverified` if semantic evidence is insufficient |
| `type "hello" into the Notes field in this isolated Playwright fixture` | screen_awareness | Playwright browser action execution | Execute replace-only `type_text` only when Playwright action gates, safe-field type gates, fresh target grounding, exact trust approval, isolated browser launch, redacted text binding, and unambiguous locator checks all pass. Append/add-more requests are unsupported. It does not press Enter or submit forms. | Yes | `browser_semantic_action_execution`; raw typed text redacted; value-summary change may support verification |
| `check the Newsletter checkbox in this isolated Playwright fixture` | screen_awareness | Playwright browser action execution | Execute `check` only when Playwright action gates, safe choice gates, fresh target grounding, exact trust approval, isolated browser launch, and unambiguous locator checks all pass. Sensitive/legal/payment/delete/security controls block, target drift blocks, already-checked no-op states are reported without issuing a browser action, and it does not submit forms. | Yes | `browser_semantic_action_execution`; checked-state comparison; submit-prevention state |
| `select Canada from the Country dropdown in this isolated Playwright fixture` | screen_awareness | Playwright browser action execution | Execute `select_option` only when Playwright action gates, safe choice gates, exact target and option approval, isolated browser launch, and bounded option extraction all pass. Missing, duplicate, disabled, sensitive, or drifted options block; stale ordinals block; already-selected no-op states are reported without issuing a browser action. It does not press Enter or submit forms. | Yes | `browser_semantic_action_execution`; selected-option comparison; bounded option summary |
| `scroll to the Privacy Policy link in this isolated Playwright fixture` | screen_awareness | Playwright browser action execution | Execute bounded `scroll_to_target` only when Playwright action gates, scroll gates, exact trust approval, isolated browser launch, and bounded target search pass. Already-visible targets are reported as no-op evidence, missing targets stop at the max attempt bound, ambiguous targets block/downgrade, and scrolling never clicks/types/selects/submits afterward. | Yes | `browser_semantic_action_execution`; target-found/not-found evidence; side-effect-prevention state |
| `prepare this isolated fixture form with safe email, Canada, and newsletter but do not submit` | screen_awareness | Playwright safe task plan | Build and execute a bounded safe task plan only when task-plan gates, every primitive step gate, exact whole-plan approval, isolated browser launch, and per-step verification pass. The ordered plan may sequence only safe primitives, redacts typed text and hidden values, stops on ambiguous/unverified/blocked steps, marks later steps skipped, and never submits the form. Tampered or replayed plans block before launch. | Yes | `browser_semantic_task_execution`; per-step semantic comparison; final no-submit verification |
| `solve the captcha` | unsupported | Planner/orchestrator | Refuse CAPTCHA, robot, or human-verification automation instead of routing to calculations, Playwright, or a safe task plan. | No | Unsupported route state |
...
| `diagnostic_only` / `protocol_probe_failed` / `endpoint_incompatible` / `cdp_navigation_unsupported` | Obscura CDP started but did not expose compatible Chrome-style endpoint/page websocket/navigation behavior. Endpoint discovery alone is diagnostics, not page-inspection success. | `src/stormhelm/core/web_retrieval/cdp.py`, `src/stormhelm/core/web_retrieval/obscura_cdp_probe.py` |
