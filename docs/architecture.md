...
| Screen awareness | Current context observation, interpretation, semantic adapter resolution, grounding, verification, gated action result, optional Playwright browser semantic readiness/mock observations, opt-in isolated Playwright semantic snapshot extraction, semantic target ranking, before/after semantic comparison, browser action previews, trust-gated click/focus execution, trust-gated safe-field `type_text`, safe choice controls, bounded scroll, and safe multi-step task plans through isolated temporary contexts only. Playwright observations feed the canonical browser adapter payload, canonical grounding uses the resulting adapter-semantic targets, and Playwright execution/task-plan results map back into canonical action-result summaries; regression tests assert planner/UI code cannot bypass that spine. | Unlimited screen control or surveillance, direct Playwright command authority, live Playwright automation in normal CI, user-profile browser control, unsafe typing/scroll/form/login/cookie/payment/CAPTCHA automation, workflow replay, learned macros, arbitrary public-site automation, visible-screen verification, truth verification, or browser action execution outside explicit trust/verification contracts. |
| Camera awareness | Explicit camera/physical-object route ownership, opt-in single-frame local camera capture, typed capability/unavailable/blocked states, bounded frame metadata, safe artifact refs when explicitly enabled, and visual-evidence-only guidance. | Continuous surveillance, background camera capture, video recording, face/identity recognition, emotion or sensitive-attribute inference, room-safety certification, screen substitution, action execution, action verification, or generic provider fallback for camera-required rows. |
...
| Windows local camera APIs | Optional live camera capture for explicit Camera Awareness paths and opt-in Kraken evaluation. Capture is bounded to single stills, released immediately, and raw frames are not exposed in events/status/reports by default. |
