...
| Camera awareness | Yes | Disabled by default; live capture is opt-in, single-frame, local, and bounded. A camera is not reported working unless a frame capture succeeds. |
...
- The Playwright browser semantic adapter lives under Screen Awareness, is disabled by default, and is capped by strict claim ceilings: `browser_semantic_observation`, `browser_semantic_observation_comparison`, `browser_semantic_action_preview`, runtime-gated `browser_semantic_action_execution`, `browser_semantic_task_plan`, and runtime-gated `browser_semantic_task_execution`.
...
- Playwright Addition 8.1 adds cross-action regression coverage for the whole implemented interaction ladder. A grant for one action kind cannot approve another action kind, tampered target/text/option/scroll metadata blocks, sensitive page contexts block click/focus/type/choice/scroll, no-submit and redaction invariants are asserted across canonical result/status/event/Deck/Ghost/audit surfaces, and provider command return is still not treated as verified success.
- Playwright Addition 9 adds safe multi-step browser task plans only behind `allow_task_plans`, `allow_dev_task_plans`, all required primitive gates, and whole-plan TrustService approval. The grant binds the exact ordered step list, every action kind, target/text/option/scroll fingerprint, expected outcomes, risk, provider, claim ceiling, and freshness; changing or reordering steps, replaying a consumed plan, or serializing away private in-memory typed text blocks execution. Every step reuses the existing primitive execution/verification path, redacts raw typed text and hidden values, and the conservative stop policy halts on blocked, failed, ambiguous, partial, unverified, unexpected navigation, sensitive target, or submit-counter evidence.
- Playwright Addition 9.1 hardens safe task plans by preserving a bounded approval-time step snapshot, blocking reordered/inserted/removed/changed/expired plans before launch, rejecting primitive-vs-plan approval crossover, marking later steps `skipped` after a stop, and keeping CAPTCHA/robot/human-verification automation on the unsupported route. It adds no new browser capabilities and keeps raw typed text, hidden values, cookies, credentials, raw DOM, and full page text out of events, status, Deck, Ghost, audit, and result surfaces.
...
- Do not use Playwright to connect to user browser sessions, type into sensitive fields, append text, fill forms, submit forms, log in, read/write cookies, download files, handle payments, bypass CAPTCHA/anti-bot controls, replay workflows, verify visible screen state, or verify source truth. Click/focus/safe-field `type_text`/safe choice-control/bounded-scroll/safe-task-plan execution exists only after explicit runtime gates and exact trust approval, only in isolated temporary contexts, and only with bounded semantic before/after comparison. Playwright command completion alone is not verification. Arbitrary public-site clicking/typing/choice-changing/scrolling/task sequencing and unrestricted browser automation remain out of scope.
...
Tests: `tests/test_screen_awareness_phase12.py`, `tests/test_screen_awareness_action.py`, `tests/test_screen_awareness_verification.py`, `tests/test_screen_awareness_playwright_adapter_scaffold.py`, `tests/test_screen_awareness_playwright_adapter_integration.py`, `tests/test_screen_awareness_playwright_canonical_kraken.py`, `tests/test_screen_awareness_playwright_live_semantic.py`, `tests/test_screen_awareness_playwright_grounding_guidance.py`, `tests/test_screen_awareness_playwright_grounding_robustness.py`, `tests/test_screen_awareness_playwright_semantic_verification.py`, `tests/test_screen_awareness_playwright_action_preview.py`, `tests/test_screen_awareness_playwright_click_focus_execution.py`, `tests/test_screen_awareness_playwright_task_plans.py`, `tests/test_live_browser_integration.py`

## Camera-Awareness Privacy

Camera Awareness is opt-in and single-frame. The live Kraken requires `STORMHELM_LIVE_CAMERA_TESTS=true` plus a live-camera gate or `--enable-live-camera`; normal CI should not run live hardware capture.

Current posture:

- Missing hardware, OS permission denial, device busy, timeout, empty frames, disabled config, or privacy blocks return typed unavailable/blocked states.
- A camera device being listed is not proof that the camera is working; a captured frame is the minimum evidence.
- Capture providers release the camera handle after each bounded capture.
- Raw frames are not logged, streamed, sent to events/status/reports, or sent to provider fallback by default.
- Frame artifacts are not persisted unless an explicit save-artifact test gate is enabled; reports use `camera-artifact:*` refs instead of raw paths or bytes.
- Camera output is visual evidence only. It cannot verify browser clicks, form submissions, downloads, room safety, prior events, or task completion.
- Face/identity recognition, emotion inference, health/age/sensitive-attribute inference, continuous watching, background monitoring, and video recording remain unsupported.

Sources: `src/stormhelm/core/camera_awareness/*`, `src/stormhelm/core/kraken/camera_awareness_live.py`, `scripts/run_camera_awareness_kraken.py`, `docs/camera-awareness.md`
Tests: `tests/test_camera_awareness_live_kraken.py`, `tests/test_camera_awareness_c8_hardening_audit.py`, `tests/test_camera_awareness_planner.py`
...
- Local-client dispatch reports step-level capability and outcome state for focus/open, Discord surface identification, DM navigation, message input targeting, payload insertion, final send gesture, and post-send verification.
...
- `sent_unverified` requires the final local send gesture. `sent_verified` additionally requires real post-send evidence that the message appeared in the target Discord thread.
...
Live/manual note: `docs/discord-relay-live-local-client.md`
